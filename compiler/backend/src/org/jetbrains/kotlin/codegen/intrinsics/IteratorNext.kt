...
public class IteratorNext : IntrinsicMethod() {
    ...
    override fun toCallable(method: CallableMethod): ExtendedCallable {
        return UnaryIntrinsic(method) {
            //val type = getThisType().toString().substringAfterLast('/').substringBefore(':')
            val returnType = getReturnType()
            val name = getIteratorName(returnType)
            it.invokevirtual(BUILT_INS_PACKAGE_FQ_NAME.toString() + "/" + name + "Iterator", "next" + name, "()" + returnType.getDescriptor(), false)
        }
    }
}
