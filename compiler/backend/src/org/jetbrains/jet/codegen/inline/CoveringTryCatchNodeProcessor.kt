...
public abstract class CoveringTryCatchNodeProcessor<in T: IntervalWithHandler>() {
    ...
    public val currentCoveringBlocks: Stack<T> = Stack()
    ...
    public fun getOpenedNodes(label: LabelNode): List<T> {
        return tryBlockStarts.get(label)
    }

    public fun getClosedNodes(label: LabelNode): List<T> {
    ...
    public open fun updateCoveringTryBlocks(curIns: AbstractInsnNode, directOrder: Boolean) {
        ...
        val infosToClose = if (!directOrder) getOpenedNodes(curIns) else getClosedNodes(curIns)
...
//            };
        ...
        val infoToOpen = if (!directOrder) getClosedNodes(curIns) else getOpenedNodes(curIns)
        for (info in Lists.reverse<T>(infoToOpen)) {
