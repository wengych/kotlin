...
class LightClassesClasspathSortingTest : KotlinCodeInsightTestCase() {
    ...
    private fun doTest(fqName: String) {
        // Same classes are in sources and in compiled Kotlin library. Test that explicit light class has a priority.
        ...
        File(testDirRoot, dirName).listFiles().forEach {
            configureByFile(it.relativeTo(testDirRoot))
        }
